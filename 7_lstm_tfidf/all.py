...
daftar_penyakit = ['Obesity', 'Diabetes Mellitus', 'Hypercholestrolemia','Hypertriglyceridemia',
	'Hypertension','Atherosclerotic CV','Heart Failure','PVD','Venous Insufficiency','Osteoarthritis','OSA','Asthma','GERD','Gallstone','Depression','Gout']

for cvk in range(0,16):
	indeks_penyakit = cvk
	print('>>>'+daftar_penyakit[indeks_penyakit]+'<<<')
	# In[2]:

	data  = pandas.read_csv('/herley/riset/wilmay/INTUITIVE2.csv')
	...
	model_checkpoint=keras.callbacks.ModelCheckpoint(daftar_penyakit[indeks_penyakit]+'_XLSTM{epoch:02d}.h5',save_weights_only=False, save_best_only=True)

	# Hilangkan tanda komentar pada history untuk memulai training.
	history = model.fit(X_train, Y_train, epochs=epochs, batch_size=batch_size,callbacks=[EarlyStopping(monitor='loss', patience=15, min_delta=0.0001), model_checkpoint], verbose=2,validation_split=0.1)
	...
	np.savetxt('prediksi_'+daftar_penyakit[indeks_penyakit]+'.txt', jawab, fmt='%d')