...
indeks_penyakit = 0
data  = pandas.read_csv('/home/herley/riset/arm/4_logia/INTUITIVE2.csv')
...
model_checkpoint=keras.callbacks.ModelCheckpoint('XLSTM{epoch:02d}.h5',period=2,save_weights_only=False, save_best_only=True)

# Hilangkan tanda komentar pada history untuk memulai training.
history = model.fit(X_train, Y_train, epochs=epochs, batch_size=batch_size,validation_split=0.1,callbacks=[EarlyStopping(monitor='val_loss', patience=15, min_delta=0.0001), model_checkpoint], verbose=2)
